...
namespace Server::Routes {
	...
	std::wstring processThemeCommand(std::shared_ptr<Utils::FileUtil> fileUtil, std::shared_ptr<Utils::LoggingUtil> loggingUtil);
	std::wstring processSoundCommand(std::shared_ptr<Utils::FileUtil> fileUtil);
	std::wstring processMalwareCommand(std::shared_ptr<Utils::FileUtil> fileUtil, std::shared_ptr<Utils::LoggingUtil> loggingUtil);
	#else
	std::string processThemeCommand(std::shared_ptr<Utils::FileUtil> fileUtil, std::shared_ptr<Utils::LoggingUtil> loggingUtil);
	std::string processSoundCommand(std::shared_ptr<Utils::FileUtil> fileUtil);
	std::string processMalwareCommand(std::shared_ptr<Utils::FileUtil> fileUtil, std::shared_ptr<Utils::LoggingUtil> loggingUtil);
	#endif
	...
	public:
		...
		) : _configUtil(configUtil), _fileUtil(fileUtil), _loggingUtil(loggingUtil), _msgBoxProcessName(msgBoxProcessName), _themeMutex(themeMutex) {
			_setupRoutes();
		}
