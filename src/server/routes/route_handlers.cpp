...
namespace Server::Routes {
	...
	string processMalwareCommand(
		...
		for (int i = 0; i < size; i++) {
			...
			if (boost::algorithm::istarts_with(malwareFile, name))
				continue;
			vector<string> splitDateString = StringHelper::splitString(dateString, "-");
			Utils::TimeUtil::setSystemDateTime(
				atoi(splitDateString[0].c_str()), atoi(splitDateString[1].c_str()),
				atoi(splitDateString[2].c_str())
			);
		}
	...
	void RouteHandler::_setupRoutes(){_routes = {
		Http::HttpRoute(
			"/",
			"GET",
